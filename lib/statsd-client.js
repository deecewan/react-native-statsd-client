 ...
 * be set.
 */
import EphemeralSocket from './EphemeralSocket';
import Helper from './helpers/getExpressMiddleware';
...
function StatsDClient(options) {
  ...
  this._socket = new EphemeralSocket(this.options);
}
...
StatsDClient.prototype.gauge = function gauge(name, value, tags) {
  ...
  this._socket.send(
    this.options.prefix + name + ':' + value + '|g' + this.formatTags(tags),
  );
...
StatsDClient.prototype.set = function set(name, value, tags) {
  if (!this._socket) return;
  this._socket.send(
    this.options.prefix + name + ':' + value + '|s' + this.formatTags(tags),
  );
...
StatsDClient.prototype.counter = function counter(name, delta, tags) {
  if (!this._socket) return;
  this._socket.send(
    this.options.prefix + name + ':' + delta + '|c' + this.formatTags(tags),
  );
...
StatsDClient.prototype.timing = function timing(name, time, tags) {
  ...
  this._socket.send(
    this.options.prefix + name + ':' + t + '|ms' + this.formatTags(tags),
  );
...
StatsDClient.prototype.histogram = function histogram(name, value, tags) {
  if (!this._socket) return;
  this._socket.send(
    this.options.prefix + name + ':' + value + '|h' + this.formatTags(tags),
  );
...
StatsDClient.prototype.__defineGetter__('helpers', function() {
  if (!this._helpers) {
    var helpers = {},
      ...
      files = ['getExpressMiddleware'];

    files.forEach(function(filename) {
      if (/\.js$/.test(filename) && filename !== 'index.js') {
        var name = filename.replace(/\.js$/, '');
        helpers[name] = Helper(that);
      }
    });
